...
int main() {
	...
	struct riffoser_wave *waveleft,*waveright;

#define writewave(wavetype,volume,frequency,pitch,filename)\
	track=riffoser_track_init(RIFFOSER_CHANNELS_STEREO);\
	waveleft=riffoser_wave_init(wavetype,volume,frequency,pitch);\
	waveright=riffoser_wave_init(wavetype,volume,frequency*2,pitch);\
	riffoser_track_addwave(track,waveleft,RIFFOSER_CHANNEL_LEFT,0,1);\
	riffoser_track_addwave(track,waveright,RIFFOSER_CHANNEL_RIGHT,1,2);\
	riffoser_track_addwave(track,waveleft,RIFFOSER_CHANNEL_LEFT,2,3);\
	riffoser_track_addwave(track,waveright,RIFFOSER_CHANNEL_RIGHT,3,5);\
	riffoser_track_addwave(track,waveleft,RIFFOSER_CHANNEL_LEFT,4,5);\
	...
	riffoser_wave_free(waveleft);\
	riffoser_wave_free(waveright);\
